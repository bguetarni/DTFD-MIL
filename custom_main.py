...
def load_data(path, args):
    ...
    for p in os.listdir(path):
        label = slide_class_idx[labels[int(p)]]
        patches = glob.glob(os.path.join(path, p, '*', '*.pt'))
        data.append((patches, label))
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier, 
    ...
    with torch.no_grad():
        ...
        for patches, label in tqdm.tqdm(mDATA_list, ncols=100):
...
def main(params):
    ...
    train_data = load_data(params.mDATA0_dir_train0, params)
